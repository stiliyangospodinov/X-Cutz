...
var ExternalChunk = class {
  ...
  getImportAttributes(snippets) {
    return this.importAttributes || (this.importAttributes = formatAttributes(this.options.format === "es" && this.options.externalImportAttributes && this.moduleInfo.attributes, snippets));
  }
...
var Variable = class extends ExpressionEntity {
  ...
  getBaseVariableName() {
    var _a;
    return ((_a = this.renderedLikeHoisted) == null ? void 0 : _a.getBaseVariableName()) || this.renderBaseName || this.renderName || this.name;
  }
  ...
  include() {
    var _a;
    this.included = true;
    (_a = this.renderedLikeHoisted) == null ? void 0 : _a.include();
  }
  /**
...
function cacheObjectGetters(object, getterProperties) {
  for (const property2 of getterProperties) {
    const propertyGetter = Object.getOwnPropertyDescriptor(object, property2).get;
    Object.defineProperty(object, property2, {
      get() {
        const value = propertyGetter.call(object);
        Object.defineProperty(object, property2, { value });
        return value;
      }
    });
  }
...
var NodeBase = class extends ExpressionEntity {
  ...
  parseNode(esTreeNode) {
    ...
    childNodeKeys[_a = esTreeNode.type] || (childNodeKeys[_a] = createChildNodeKeysForNode(esTreeNode));
    this.initialise();
    return this;
  }
  ...
  CustomEvent: {
    ...
    [ValueProperties]: {
      deoptimizeArgumentsOnCall({ args }) {
        var _a;
        (_a = args[2]) == null ? void 0 : _a.deoptimizePath(["detail"]);
      },
      getLiteralValue: getTruthyLiteralValue,
      hasEffectsWhenCalled: returnFalse
    },
...
var tdzVariableKinds = /* @__PURE__ */ new Set(["class", "const", "let", "var", "using", "await using"]);
var Identifier2 = class extends NodeBase {
  ...
  isPossibleTDZ() {
    ...
    if (!(this.variable instanceof LocalVariable && this.variable.kind && tdzVariableKinds.has(this.variable.kind) && // we ignore possible TDZs due to circular module dependencies as
...
var ParameterVariable = class extends LocalVariable {
  ...
  getLiteralValueAtPath(path2, recursionTracker, origin) {
    ...
    return recursionTracker.withTrackedEntityAtPath(path2, knownValue, () => knownValue.getLiteralValueAtPath(path2, recursionTracker, origin), UnknownValue);
  }
  hasEffectsOnInteractionAtPath(path2, interaction, context) {
    if (this.isReassigned || interaction.type === INTERACTION_ASSIGNED) {
      return super.hasEffectsOnInteractionAtPath(path2, interaction, context);
    }
    const knownValue = this.getKnownValue();
    return knownValue.hasEffectsOnInteractionAtPath(path2, interaction, context);
  }
  deoptimizeArgumentsOnInteractionAtPath(interaction, path2) {
...
function getSafeName(baseName, usedNames, forbiddenNames) {
  ...
  let count = 1;
  while (usedNames.has(safeName) || RESERVED_NAMES.has(safeName) || (forbiddenNames == null ? void 0 : forbiddenNames.has(safeName))) {
...
var Scope = class {
  ...
  addDeclaration(identifier2, context, init2, kind) {
    ...
    const existingVariable = ((_a = this.hoistedVariables) == null ? void 0 : _a.get(name)) || this.variables.get(name);
    if (existingVariable) {
  ...
  addHoistedVariable(name, variable) {
    (this.hoistedVariables || (this.hoistedVariables = /* @__PURE__ */ new Map())).set(name, variable);
  }
  contains(name) {
...
var FunctionBase = class extends NodeBase {
  ...
  onlyFunctionCallUsed() {
    ...
    return (variable == null ? void 0 : variable.getOnlyFunctionCallUsed()) ?? false;
  }
  include(context, includeChildrenRecursively) {
...
var AssignmentExpression = class extends NodeBase {
  ...
  initialise() {
    ...
    if (this.left instanceof Identifier2) {
      ...
      if ((variable == null ? void 0 : variable.kind) === "const") {
...
var FunctionDeclaration = class extends FunctionNode {
  ...
  onlyFunctionCallUsed() {
    var _a;
    return ((_a = this.id) == null ? void 0 : _a.variable.getOnlyFunctionCallUsed()) ?? super.onlyFunctionCallUsed();
  }
  parseNode(esTreeNode) {
    if (esTreeNode.id !== null) {
      this.id = new Identifier2(this, this.scope.parent).parseNode(esTreeNode.id);
...
var ExportDefaultDeclaration2 = class extends NodeBase {
  include(context, includeChildrenRecursively) {
    super.include(context, includeChildrenRecursively);
    if (includeChildrenRecursively) {
      this.scope.context.includeVariableInModule(this.variable);
...
var ExportNamedDeclaration = class extends NodeBase {
  ...
  removeAnnotations(code) {
    var _a;
    (_a = this.declaration) == null ? void 0 : _a.removeAnnotations(code);
  }
  render(code, options, nodeRenderOptions) {
    const { start, end } = nodeRenderOptions;
...
var VariableDeclarator2 = class extends NodeBase {
  ...
  removeAnnotations(code) {
    var _a;
    (_a = this.init) == null ? void 0 : _a.removeAnnotations(code);
  }
  render(code, options) {
...
var MetaProperty = class extends NodeBase {
  ...
  getReferencedFileName(outputPluginDriver) {
    ...
    if (name === IMPORT && (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_PREFIX))) {
  ...
  include() {
    if (!this.included) {
      ...
      if (this.meta.name === IMPORT) {
        ...
        if (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_PREFIX)) {
  ...
  render(code, renderOptions) {
    ...
    if (!replacement) {
      replacement = (_a = importMetaMechanisms[format]) == null ? void 0 : _a.call(importMetaMechanisms, metaProperty2, { chunkId, snippets });
...
var getFileUrlFromRelativePath = (path2) => getFileUrlFromFullPath(`__dirname + '/${escapeId(path2)}'`);
...
var PanicError = class extends NodeBase {
  initialise() {
    ...
    const parseError2 = getRollupError(logParseError(this.message));
...
var ParseError = class extends NodeBase {
  initialise() {
    ...
    const id = this.scope.context.module.id;
    const parseError2 = getRollupError(logParseError(this.message, pos));
    const moduleParseError = logModuleParseError(parseError2, id);
...
var TryStatement = class extends NodeBase {
  ...
  hasEffects(context) {
    var _a;
    return (this.scope.context.options.treeshake.tryCatchDeoptimization ? this.block.body.length > 0 : this.block.hasEffects(context)) || !!((_a = this.finalizer) == null ? void 0 : _a.hasEffects(context));
  }
  include(context, includeChildrenRecursively) {
    var _a, _b;
    const tryCatchDeoptimization = (_a = this.scope.context.options.treeshake) == null ? void 0 : _a.tryCatchDeoptimization;
    const { brokenFlow, includedLabels } = context;
...
function gatherSystemExportsAndGetSingleExport(separatedNodes, options, aggregatedSystemExports) {
  ...
  if (options.format === "system") {
    for (const { node } of separatedNodes) {
      if (node.id instanceof Identifier2 && node.init && aggregatedSystemExports.length === 0 && ((_a = options.exportNamesByVariable.get(node.id.variable)) == null ? void 0 : _a.length) === 1) {
...
var bufferParsers = [
  ...
  function assignmentExpression(node, position, buffer, readString) {
    ...
    node.left = convertNode2(node, scope, buffer[position + 1], buffer, readString);
    node.right = convertNode2(node, scope, buffer[position + 2], buffer, readString);
  },
  function assignmentPattern(node, position, buffer, readString) {
    const { scope } = node;
    node.left = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function awaitExpression(node, position, buffer, readString) {
    const { scope } = node;
    node.argument = convertNode2(node, scope, buffer[position], buffer, readString);
  },
  ...
  function callExpression(node, position, buffer, readString) {
    ...
    node.callee = convertNode2(node, scope, buffer[position + 2], buffer, readString);
  ...
  function catchClause(node, position, buffer, readString) {
    ...
    parameter == null ? void 0 : parameter.declare("parameter", UNKNOWN_EXPRESSION);
  ...
  function conditionalExpression(node, position, buffer, readString) {
    ...
    node.consequent = convertNode2(node, scope, buffer[position + 1], buffer, readString);
    node.alternate = convertNode2(node, scope, buffer[position + 2], buffer, readString);
  },
  ...
  function directive(node, position, buffer, readString) {
    ...
    node.expression = convertNode2(node, scope, buffer[position + 1], buffer, readString);
  },
  function doWhileStatement(node, position, buffer, readString) {
    ...
    node.test = convertNode2(node, scope, buffer[position + 1], buffer, readString);
  },
  ...
  function exportDefaultDeclaration(node, position, buffer, readString) {
    const { scope } = node;
    node.declaration = convertNode2(node, scope, buffer[position], buffer, readString);
  },
  ...
  function exportSpecifier(node, position, buffer, readString) {
    const { scope } = node;
    node.local = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function functionDeclaration(node, position, buffer, readString) {
    ...
    scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement);
    node.body = convertNode2(node, scope.bodyScope, buffer[position + 4], buffer, readString);
  },
  ...
  function ifStatement(node, position, buffer, readString) {
    const { scope } = node;
    node.test = convertNode2(node, scope, buffer[position], buffer, readString);
    node.consequent = convertNode2(node, node.consequentScope = new TrackingScope(scope), buffer[position + 1], buffer, readString);
  ...
  function importAttribute(node, position, buffer, readString) {
    const { scope } = node;
    node.key = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function importExpression(node, position, buffer, readString) {
    ...
    node.sourceAstNode = convertNode(buffer[position], buffer, readString);
  ...
  function labeledStatement(node, position, buffer, readString) {
    const { scope } = node;
    node.label = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function memberExpression(node, position, buffer, readString) {
    ...
    node.object = convertNode2(node, scope, buffer[position + 1], buffer, readString);
    node.property = convertNode2(node, scope, buffer[position + 2], buffer, readString);
  },
  function metaProperty(node, position, buffer, readString) {
    const { scope } = node;
    node.meta = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function methodDefinition(node, position, buffer, readString) {
    ...
    node.key = convertNode2(node, scope, buffer[position + 1], buffer, readString);
  ...
  function switchStatement(node, position, buffer, readString) {
    const { scope } = node;
    node.discriminant = convertNode2(node, node.parentScope, buffer[position], buffer, readString);
  ...
  function taggedTemplateExpression(node, position, buffer, readString) {
    const { scope } = node;
    node.tag = convertNode2(node, scope, buffer[position], buffer, readString);
    node.quasi = convertNode2(node, scope, buffer[position + 1], buffer, readString);
  },
  ...
  function tryStatement(node, position, buffer, readString) {
    const { scope } = node;
    node.block = convertNode2(node, scope, buffer[position], buffer, readString);
  ...
  function unaryExpression(node, position, buffer, readString) {
    const { scope } = node;
    node.operator = FIXED_STRINGS[buffer[position]];
    node.argument = convertNode2(node, scope, buffer[position + 1], buffer, readString);
  },
  function updateExpression(node, position, buffer, readString) {
    ...
    node.argument = convertNode2(node, scope, buffer[position + 2], buffer, readString);
  },
  ...
  function variableDeclarator(node, position, buffer, readString) {
    const { scope } = node;
    node.id = convertNode2(node, scope, buffer[position], buffer, readString);
...
function getAttributesFromImportExpression(node) {
  ...
  const assertProperty = (_a = options.properties.find((property2) => ATTRIBUTE_KEYWORDS.has(getPropertyKey(property2)))) == null ? void 0 : _a.value;
  if (!assertProperty) {
    return EMPTY_OBJECT;
  }
...
function getAttributesFromImportExportDeclaration(attributes) {
  return (attributes == null ? void 0 : attributes.length) ? Object.fromEntries(attributes.map((assertion) => [getPropertyKey(assertion), assertion.value.value])) : EMPTY_OBJECT;
}
...
var Module = class _Module {
  ...
  getDependenciesToBeIncluded() {
    ...
    if (this.info.isEntry || this.includedDynamicImporters.length > 0 || this.namespace.included || this.implicitlyLoadedAfter.size > 0) {
      for (const exportName of [...this.getReexports(), ...this.getExports()]) {
        ...
        if (exportedVariable == null ? void 0 : exportedVariable.included) {
  ...
  getRenderedExports() {
    ...
    for (const exportName of this.exports.keys()) {
      ...
      ((variable == null ? void 0 : variable.included) ? renderedExports : removedExports).push(exportName);
    }
      ...
      if (addNonNamespacesAndInteropHelpers || variable.isNamespace) {
        ...
        if (module instanceof ExternalModule) {
          ...
          if (addNonNamespacesAndInteropHelpers) {
            ...
            } else if (variable.isNamespace && namespaceInteropHelpersByInteropType[interop(module.id)] && (this.imports.has(variable) || !((_a = this.exportNamesByVariable.get(variable)) == null ? void 0 : _a.every((name) => name.startsWith("*"))))) {
    ...
    for (const module of orderedModules) {
      ...
      if (module.isIncluded() || includedNamespaces.has(module)) {
        ...
        if (!renderOptions.accessedDocumentCurrentScript && formatsMaybeAccessDocumentCurrentScript.includes(format)) {
          (_a = this.accessedGlobalsByScope.get(module.scope)) == null ? void 0 : _a.delete(DOCUMENT_CURRENT_SCRIPT);
        }
...
function getChunksWithSameDependentEntriesAndCorrelatedAtoms(chunkAtoms, staticDependencyAtomsByEntry, alreadyLoadedAtomsByEntry, minChunkSize) {
  ...
  for (const { dependentEntries, modules } of chunkAtoms) {
    ...
    const chunk = chunksBySignature[_a = String(chunkSignature)] || (chunksBySignature[_a] = {
      containedAtoms: 0n,
      correlatedAtoms,
...
async function transformChunksAndGenerateContentHashes(renderedChunks, chunkGraph, outputOptions, pluginDriver, getHash, log) {
  ...
  await Promise.all(renderedChunks.map(async ({ chunk, preliminaryFileName: { fileName, hashPlaceholder }, preliminarySourcemapFileName, magicString, usedModules }) => {
    const transformedChunk = {
      chunk,
      fileName,
      sourcemapFileName: (preliminarySourcemapFileName == null ? void 0 : preliminarySourcemapFileName.fileName) ?? null,
    ...
    const sourcemapHashPlaceholder = preliminarySourcemapFileName == null ? void 0 : preliminarySourcemapFileName.hashPlaceholder;
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
  ...
  if (sourcemap === "inline") {
    ...
    const sourcemapFileName = (0, import_node_path.basename)(fileName);
...
var defaultPrintLog = (level, { message }) => {
  switch (level) {
    case LOGLEVEL_WARN: {
  ...
  async addModuleSource(id, importer, module) {
    ...
    try {
      source = await this.graph.fileOperationQueue.run(async () => {
        ...
        return await (0, import_promises.readFile)(id, "utf8");
      });
    } catch (error_) {
...
` + [...unfulfilledActions].map(formatAction).join("\n")));
      ...
      import_node_process.default.once("beforeExit", handleBeforeExit);
    }
  });
  try {
    ...
    if (rejectByPluginDriver.size === 0) {
      import_node_process.default.off("beforeExit", handleBeforeExit);
...
var stderr = (...parameters) => import_node_process.default.stderr.write(`${parameters.join("")}
`);
...
`, "")));
  ...
  if (error2.cause) {
    ...
    while (cause) {
      indent += "  ";
      const message2 = cause.stack || cause;
      causeErrorLines.push(...`[cause] ${message2}`.split("\n").map((line) => indent + line));
      cause = cause.cause;
    }
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-UBS3WQWH.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-J2IJMXAC.js.map
